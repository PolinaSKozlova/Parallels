...
public:
 ...
 public:
  ThreadsPool(unsigned const threads_count): done_(false),joiner_(threads_) {
    // unsigned const threads_count = std::thread::hardware_concurrency(); // think about
    try {
...
template <typename FunctionType>
 ...
 void Run() {
  ...
  if(work_queue_.TryPop(result_task)){
    ...
    std::cout << "Run  std::this_thread::yield(); " <<  std::this_thread::get_id() << std::endl;
