...
namespace Parallels
{
    ...
    public:
        Ant(int position, const Matrix &distances, const Matrix &pheromones, double alpha = 1.0, double beta = 1.0)
    ...
    {
        return _pheromones[_position][direction];
    }
    ...
    void one_way_tour()
    {
        ...
        {
            int next_direction = next_direction(true);
            if (next_direction != -1)
            {
            ...
            {
                next_direction = next_direction(false);
                travel(next_direction);
            }
        }
    }
    ...
    double get_route_length()
    {
        if (_route.size() > 1)
        {
            ...
            {
                route_length += _distances[_route[idx]][_route[idx + 1]];
            }
            return route_length;
        }
        else
    ...
    {
        int n_vertices = _distances.size();
        double Q = static_cast<double>(n_vertices);
        double route_length = get_route_length();
        double additive = Q / route_length;

        std::vector<std::vector<double>> additives(n_vertices, std::vector<double>(n_vertices, 0.0));
